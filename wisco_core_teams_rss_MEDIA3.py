...
#!/usr/bin/env python3
Wisconsin Sports Aggregator RSS — Packers, Bucks, Brewers, Badgers (MEDIA3)
-------------------------------------------------------------------------------
...
- Images for score items:
    1) ESPN JSON team logos/photos
    2) (optional) scrape og:image from event link
    3) fallback logos (if provided)
- Emits both <enclosure> and <media:content>; stable GUIDs to avoid image bleed.

Usage (local test):
  python wisco_core_teams_rss_MEDIA3.py --out wisconsin-sports.xml --days 6 --max-items 240 --live-final-only --prefer-event-photos --scrape-og --fallback-base-url "https://<user>.github.io/wisconsin-sports-feed"
...
def first_img_in_html(html: str) -> str | None:
